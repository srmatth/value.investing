...
random_forest_binary <- function(.data, industry) {
  ...
  grid <- list(
    ...
    min_rows = c(1),
    mtries = c(4, 7),
    min_split_improvement = c(.0001, .001),
    sample_rate = c(.632),
    nbins = c(4, 20),
...
gradient_boosted_binary <- function(.data, industry) {
  ...
  for (i in 1:nrow(grid)) {
    ...
    tryCatch({
      ...
      tmp_mod <- h2o::h2o.gbm(
        ...
        learn_rate_annealing = grid_sub$learn_rate_annealing,
        min_split_improvement = grid_sub$min_split_improvement,
        sample_rate = grid_sub$sample_rate,
        nbins = grid_sub$nbins,
        col_sample_rate_per_tree = grid_sub$col_sample_rate_per_tree,
  ...
  final_mod_f1 <- h2o::h2o.gbm(
    ...
    max_depth = best_model_f1$max_depth,
    min_rows = best_model_f1$min_rows,
    distribution = best_model_f1$distribution,
    learn_rate = best_model_f1$learn_rate,
    learn_rate_annealing = best_model_f1$learn_rate_annealing,
    min_split_improvement = best_model_f1$min_split_improvement,
    sample_rate = best_model_f1$sample_rate,
    nbins = best_model_f1$nbins,
    col_sample_rate_per_tree = best_model_f1$col_sample_rate_per_tree,
  ...
  final_mod_98 <- h2o::h2o.gbm(
    ...
    max_depth = best_model_98$max_depth,
    min_rows = best_model_98$min_rows,
    distribution = best_model_98$distribution,
    learn_rate = best_model_98$learn_rate,
    learn_rate_annealing = best_model_98$learn_rate_annealing,
    min_split_improvement = best_model_98$min_split_improvement,
    sample_rate = best_model_98$sample_rate,
    nbins = best_model_98$nbins,
    col_sample_rate_per_tree = best_model_98$col_sample_rate_per_tree,
    histogram_type = best_model_98$histogram_type
  )
  ...
  readr::write_csv(results, paste0(path, "/gb_prob_tuning_results.csv"))
  readr::write_csv(all_preds, paste0(path, "/gb_prob_preds.csv"))
}
...
gradient_boosted_growth <- function(.data, industry) {
  ...
  final_mod <- h2o::h2o.gbm(
    y = "three_year_growth",
    training_frame = train,
    nfolds = 5,
    model_id = "final_gb_growth_mod",
    ntrees = best_model$ntrees,
    max_depth = best_model$max_depth,
    min_rows = best_model$min_rows,
    distribution = best_model$distribution,
    learn_rate = best_model$learn_rate,
    learn_rate_annealing = best_model$learn_rate_annealing,
    min_split_improvement = best_model$min_split_improvement,
    sample_rate = best_model$sample_rate,
    nbins = best_model$nbins,
    col_sample_rate_per_tree = best_model$col_sample_rate_per_tree,
    histogram_type = best_model$histogram_type
  )
  ...
  readr::write_csv(results, paste0(path, "/gb_growth_tuning_results.csv"))
  readr::write_csv(preds, paste0(path, "/gb_growth_preds.csv"))
  readr::write_csv(explain_growth, paste0(path, "/gb_growth_explained.csv"))
}
